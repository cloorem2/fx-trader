...
async function doGotPrice() {
  ...
  if (macd > amacd) {
    if (macd_key == -1) {
      // if (pos[pname] == 0) {
        ...
        const sl = (current_ask - long_sl - aspread).toFixed(precission);
        if (trade_control == 1)
  ...
  if (macd < amacd) {
    if (macd_key == 1) {
      if (pos[pname] == 0) {
          ...
          const sl = (current_bid - shrt_sl + aspread).toFixed(precission);
        if (trade_control == 1)
          await doMarketOrder(-size,tp,sl,pname);
  ...
  fs.writeFileSync('amidps0-' + pname,amidps0 + '\n');
  fs.writeFileSync('amidps1-' + pname,amidps1 + '\n');
  fs.writeFileSync('amacd-' + pname,amacd + '\n');
  return;
