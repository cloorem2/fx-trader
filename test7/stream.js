...
async function doTransData(data) {
  ...
  transTimeout = setTimeout(() => { doTransactions(); }, 20_000);
...
async function doChunk(data) {
  ...
  mainTimeout = setTimeout(() => { doMain(); }, 20_000);
  ...
  current_time = t2 + t3 + t4 + t6 + t7 + t9 + t10;
  ...
  if (t6 != ot6) {
    ...
    for (var i = 0; i < num_strats; i++) {
      const t = tleg_id[i];
      fstr += maxb_t[t] + ' ' + maxa_t[t] + ' ' + dir_t[t] + '\n';
    }
...
async function doMarketOrder(size,tpname) {
...
var transTimeout;
          ...
          mutex = mutex.then(async () => {
            ...
            console.log(maxb_t);
            console.log(maxa_t);
            console.log('odir_key',odir_key,'prof_t',prof_t[odir_key]);
            // await doChunk(data);
