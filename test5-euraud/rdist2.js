...
/*
try {
  console.log('financing',pname);
  const j = fs.readFileSync('../financing.txt','utf8');
  const jj = j.split('\n').join('').split(' ').join('');
  console.log(jj);
  const jjj = JSON.parse(jj);
  console.log(jjj);
  if (j.name == pname) {
    console.log('financing',pname);
    console.log(j.marginRate,j.financing.longRate,j.financing.shortRate);
  }
} catch {}
*/
...
async function doMain() {
  ...
  while (true) {
    ...
    const tick_file = await fsPromises.open('../data-euraud/ticks-hist');
    ...
    if (smode == 0) {
      ...
      if (tlevx > 1 / marginRate) tlevx = 1 / marginRate;
      profit_x1 = profit_x;
    ...
    if (profit_x >= profit_x1) {
      ...
      if (profit_c[rmode] > 20) profit_c[rmode] = 20;
    }
