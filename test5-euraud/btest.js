...
async function doMain() {
  ...
  var back_profit1 = 0;
  var back_profit2 = 0;
  var bp0 = Number(fs.readFileSync('bp0','utf8'));
  while (true) {
    ...
    bp0 *= 0.99;
    bp0 += back_profit / 100;
    fs.writeFileSync('bp0',bp0.toExponential(9) + '\n');
    if (back_profit > bp0) {
      opt_a *= 0.9;
      opt_a += topt_a / 10;
      opt_b *= 0.9;
      opt_b += topt_b / 10;
      fs.writeFileSync('opt_a',opt_a.toExponential(9) + '\n');
      fs.writeFileSync('opt_b',opt_b.toExponential(9) + '\n');
    }
    var tstr = ' bp ';
    if (back_profit >= 0) tstr += ' ';
    tstr += back_profit.toExponential(3);
    if (back_profit >= 0) tstr += ' ';
    tstr += ' ' + (back_profit/back_count).toExponential(3);
    tstr += ' bp0 ';
    if (bp0 >= 0) tstr += ' ';
    tstr += bp0.toExponential(3)
        + ' a ' + topt_a.toExponential(3)
        + ' b ' + topt_b.toExponential(3)
        + ' ' + back_count;
    console.log(tstr);
    topt_a = opt_a * (1 + (2 * Math.random() - 1) / 10);
    topt_b = opt_b * (1 + (2 * Math.random() - 1) / 10);
    continue;
    ...
    if (ncutx < ncut[0]) {
      ...
      */
      ...
      while (tt[tstr] == 1) {
        ...
        if (tvv_poss[maxi][maxii] / tvv_posc[maxi][maxii] > 0.9 * aspread) {
          if (Math.random() < 0.9) tvv_pos[maxi][maxii] = 1;
          else tvv_pos[maxi][maxii] = 3;
        } else if (tvv_poss[maxi][maxii] / tvv_posc[maxi][maxii] > 0.1 * aspread) {
          if (Math.random() <
            tvv_poss[maxi][maxii] / tvv_posc[maxi][maxii] / aspread
          ) tvv_pos[maxi][maxii] = 1;
          else tvv_pos[maxi][maxii] = 3;
        } else if (tvv_poss[maxi][maxii] / tvv_posc[maxi][maxii] > -0.1 * aspread) {
          if (Math.random() < 0.1) tvv_pos[maxi][maxii] = 1;
          else if (Math.random() < 0.1) tvv_pos[maxi][maxii] = 2;
          else tvv_pos[maxi][maxii] = 3;
        } else if (tvv_poss[maxi][maxii] / tvv_posc[maxi][maxii] > -0.9 * aspread) {
          if (Math.random() <
            -tvv_poss[maxi][maxii] / tvv_posc[maxi][maxii] / aspread
...
var opt_a = Number(fs.readFileSync('opt_a','utf8'));
var opt_b = Number(fs.readFileSync('opt_b','utf8'));
var topt_a = opt_a;
var topt_b = opt_b;
async function doTickLine(line) {
  ...
  if (back_price == 0) {
    back_price = midp;
    dir = 2 * Math.floor(2 * Math.random()) - 1;
    if (dir == 1) back_price = current_ask;
    else back_price = current_bid;
  }
  const x = midp - back_price;
  if (dir == 1) {
    if (x > 0) {
      if (Math.random() < topt_a) {
