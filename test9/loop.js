...
async function doMain() {
  while (true) {
    ...
    for (var i = 0; i < x_len; i++) {
      ...
      console.log('                 prof/d   llevx   slevx    macdc    ma0c    ma1c    ltp     lsl      stp     ssl');
      for (var iii = 0; iii < pair_l.length; iii++) {
        ...
        tstr += ' ' + Number(ttstr[2]).toExponential(2);
        tstr += ' ' + Number(ttstr[10]).toExponential(2);
        ...
        tstr += '  ' + Number(ttstr[8]).toExponential(2);
        tstr += ' ' + Number(ttstr[9]).toExponential(2);
