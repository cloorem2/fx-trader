...
async function doChunk(data) {
  ...
  if (candle_done == 1) await doMadeDelayBrain(midp,pvar);
...
async function doTrade(midp,pvar) {
  var tlevx = levx;
  if (dnav < 0) tlevx /= 10;
  buy_size = Number((tlevx * 50 * nav / midp).toFixed());
  sell_size = buy_size;
...
async function doMadeDelayBrain(midp,pvar) {
  ...
  if (linec == 0) {
    console.log('    pvar    bids    asks     nav      adnav    prof_t   utc');
  }
  ...
  var tstr =
    ...
    ;
  if (prof_t >= 0) tstr += ' ';
  tstr += ' ' + prof_t.toExponential(2);
  tstr += ' ' + time_str;
  if (trade_str != '') tstr += ' --- ' + trade_str;
