...
try {
  ...
  for (var i of d.split('\n')) {
    ...
    // if (ii.length == 3 + num_strats) {
      ...
      tpx_t[tc] = Number(ii[6]);
      if (tpx_t[tc] > 1) tpx_t[tc] = 1;
      slx_t[tc] = Number(ii[7]);
      if (slx_t[tc] > 1) slx_t[tc] = 1;
...
async function doMain() {
  ...
  while (true) {
    ...
    */
    ...
    const profit_x = sl_count1 < 3 ? 0
      : Math.exp(Math.log(balance)/elapsed_days) - 1;
    ...
    while (trmode > 0) {
      if (profit_t[trmode] > profit_t[trmode - 1]) {
        ...
        const px = tpx_t[trmode];
        tpx_t[trmode] = tpx_t[trmode - 1];
        tpx_t[trmode - 1] = px;
        const sx = slx_t[trmode];
        slx_t[trmode] = slx_t[trmode - 1];
        slx_t[trmode - 1] = sx;
    ...
    console.log(
      ...
      + ' tpx ' + ttpx.toExponential(2)
      + ' slx ' + tslx.toExponential(2)
      + ' ' + tamidps0c.toFixed(1) + ' ' + tamidps1c.toFixed(1)
      + '   ---   ' + pname
    );
    console.log('amacdc',tamacdc.toFixed(2), 'up',
    ...
    if (rmode == 0) {
      console.log('arsi',arsi.toExponential(3));
    ...
    for (var i = 0; i < num_final; i++) {
      fstr += profit_c[i] + ' ' + profit_t[i].toExponential(9)
        ...
        + ' ' + tpx_t[i].toExponential(9)
        + ' ' + slx_t[i].toExponential(9)
        // + ' ' + final_leg_t[i].join(' ')
        + '\n';
      ...
      tstr += ' ' + profit_t[i].toExponential(3)
        ...
        + '  -- ' + tpx_t[i].toExponential(3)
        + ' ' + slx_t[i].toExponential(3)
        ;
      console.log(tstr);
    }
    ...
    if ((rmode == num_final - 1)
      ...
      if ((rand_x < 0.1) || (profit_t[0] <= 0)) {
        amidps1c_t[rmode] = amidps1c_t[0] * 20 * Math.random();
        amidps0c_t[rmode] = amidps1c_t[rmode] * Math.random();
        amacdc_t[rmode] = amidps0c_t[rmode] * Math.random();
        tpx_t[rmode] = 9 * Math.random();
        slx_t[rmode] = 9 * Math.random();
      } else if (rand_x < 0.6) {
        ...
        tpx_t[rmode] = tpx_t[0] * (1 + (2 * Math.random() - 1) / 10);
        slx_t[rmode] = slx_t[0] * (1 + (2 * Math.random() - 1) / 10);
      } else if (rand_x < 0.9) {
        ...
        tpx_t[rmode] = tpx_t[0] * (1 + (2 * Math.random() - 1) / 100);
        slx_t[rmode] = slx_t[0] * (1 + (2 * Math.random() - 1) / 100);
      } else {
        ...
        if (xx < 0.2)
          ...
          tpx_t[rmode] = tpx_t[0] * 9 * Math.random();
        else
          slx_t[rmode] = slx_t[0] * 9 * Math.random();
      }
      tamidps1c = amidps1c_t[rmode];
      tamidps0c = amidps0c_t[rmode];
      tamacdc = amacdc_t[rmode];
      if (tpx_t[rmode] > 1) tpx_t[rmode] = 1;
      if (slx_t[rmode] > 1) slx_t[rmode] = 1;
      ttpx = tpx_t[rmode];
      tslx = slx_t[rmode];
      tlevx = levx_t[rmode];
      /*
...
var amacd = 0;
// var amacdc = Number(fs.readFileSync('amacdc-' + pname,'utf8'));
...
async function doTickLine(line) {
  ...
  if (base_pos > 0) {
    ...
    if (current_bid > base_price + amacd_max_pd1 * ttpx) {
      base_profit += (current_bid - base_price) / base_price;
      balance += base_size * (current_bid - base_price);
    ...
    if (current_ask < base_price - amacd_max_pd1 * tslx) {
      const cb = Number((base_price - amacd_max_pd1 * tslx - aspread)
        .toFixed(precission));
    ...
    if (current_ask < base_price + amacd_max_pd0 * ttpx) {
      base_profit += (base_price - current_ask) / current_ask;
      balance += base_size * (base_price - current_ask);
    ...
    if (current_bid > base_price - amacd_max_pd0 * tslx) {
      const ca = Number((base_price - amacd_max_pd0 * tslx + aspread)
        .toFixed(precission));
